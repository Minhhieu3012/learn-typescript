...
const addNewRowWithJS = (user) => {
  ...
  `;
    ...
    const btnDeleteElement = document.querySelector(`.delete-user[data-delete-id="${user.id}"]`);
    if (btnDeleteElement) {
        actionClickDelete(btnDeleteElement);
    }
    //Assign edit event for new line
    const btnEditElement = document.querySelector(`.edit-user[data-update-id="${user.id}"]`);
    if (btnEditElement) {

...
const addNewRowWithJS = (post) => {
  ...
  `;
    ...
    const btnDeleteElement = document.querySelector(`.delete-post[data-delete-id="${post.id}"]`);
    if (btnDeleteElement) {
        actionClickDelete(btnDeleteElement);
    }
    //Assign edit event for new line
    const btnEditElement = document.querySelector(`.edit-post[data-update-id="${post.id}"]`);
    if (btnEditElement) {
...
const submitModalCreate = () => {
    ...
    if (btnCreate) {
        btnCreate.addEventListener("click", async () => {
            const titleInput = document.getElementById("titleCreate");
            const contentInput = document.getElementById("contentCreate");
